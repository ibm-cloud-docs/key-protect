...
## Considerations for a key using dual authorization
...
- **Determine who can authorize deletion of your {{site.data.keyword.keymanagementserviceshort}} resources.**
  ...
  deletion and another who can delete the key. Users with a _Writer_ or
  _Manager_ role can set keys for deletion. Users with a _Manager_
  role can delete keys.
- **Plan to delete the key within a seven-day waiting period.**
  When the first user authorizes a key for deletion,
  {{site.data.keyword.keymanagementserviceshort}} applies a seven-day waiting period on
  ...
  second user with a _Manager_ role can use the
  {{site.data.keyword.keymanagementserviceshort}} GUI or API to delete the key.
- **The key and its associated data will be inaccessible 90 days after being deleted.**
  When you delete a key, it is only soft deleted, meaning that the key and its
  associated data will be restorable up to 30 days after deletion. You are able
  to still retrieve associated data such as key metadata, registrations,
  and policies for up to 90 days. After 90 days, the key becomes eligible to be
  automatically [purged](#delete-dual-auth-keys-key-purge), or hard deleted, and its associated
  data will be permanently removed from the
...
you can provide the first authorization to delete a key by
...
8. Contact the second second approver to complete the deletion of the key.

The second approver must have _Manager_ access policy for the instance or key in
...
{{site.data.keyword.keymanagementserviceshort}} sets a seven-day waiting period that
...
If you need to cancel an authorization for a key before the seven-day waiting period
