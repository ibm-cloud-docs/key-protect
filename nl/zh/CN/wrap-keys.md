...
在提供用于打包的根密钥时，请确保根密钥为 256 位、384 位或 512 位，这样打包调用才能成功。如果在服务中创建根密钥，那么 {{site.data.keyword.keymanagementserviceshort}} 将从其 HSM 生成 AES-GCM 算法支持的 256 位密钥。
{: note}

[在服务中指定根密钥后](/docs/services/key-protect/create-root-keys.html)，可以通过对以下端点发出 `POST` 调用以高级加密方式来打包 DEK。
