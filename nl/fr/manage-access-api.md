...
Pour utiliser l'API, générez vos données d'authentification, par exemple, votre [jeton d'accès](/docs/services/key-protect/access-api.html#retrieve-token) et  votre [ID d'instance](/docs/services/key-protect/access-api.html#retrieve-instance-ID). Vous avez également besoin de l'ID de la clé {{site.data.keyword.keymanagementserviceshort}} dont vous souhaitez gérer l'accès. 

Pour savoir comment afficher les ID de clé, voir [Affichage des clés](/docs/services/key-protect/view-keys.html).
{: tip} 
...
1. Connectez-vous à l'interface de ligne de commande de {{site.data.keyword.cloud_notm}}.
    ```sh
    ibmcloud login [--sso]
    ```
    {: codeblock}
...
1. [Demandez à l'utilisateur de fournir son jeton IAM](/docs/services/key-protect/access-api.html#retrieve-token).
