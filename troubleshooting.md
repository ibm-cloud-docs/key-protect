...
interface, the option to add a key to the service instance is disabled..
...
applicable service instance. For more information about roles, see
...
keys in a service instance that you have access to.
...
Contact an administrator to check your permissions. If the service instance
...
in the service instance. If the service instance contains more than 200 keys,
you need to use the
...
to the service instance that you're targeting.

The service instance contains a large number of keys, and the specific keys that
...
service instance contains more than 200 keys, you need to use the
[`offset` and `limit` parameters](/docs/key-protect?topic=key-protect-view-keys#retrieve-subset-keys-api)
to list another subset of keys.
{: tsResolve}
...
You're assigned a _Manager_ access policy for the service instance. You try to
